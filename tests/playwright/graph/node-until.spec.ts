...
/**
 * The current demo workflow (workflow-root.ts) ships no `core.loop.until`
...
async function addUntilNode(page: Page): Promise<void> {
  await page.locator('button.graph-renderer__palette-btn').click();
  const entry = page
    .locator('.graph-renderer__palette-item')
    .filter({ hasText: 'GraphUntilLoopNode' })
    .first();
  await expect(entry).toBeVisible();
  await entry.click();
  await expect(getNodeArticle(page, UNTIL)).toBeVisible({ timeout: 10_000 });
  await page.waitForTimeout(500);
}

test.describe('GraphUntilLoopNode (core.loop.until), palette-added', () => {
  test.setTimeout(180_000);

  test.beforeEach(async ({ page }) => {
    await gotoGraph(page);
    await addUntilNode(page);
  });

  test('renders with the manifest display name', async ({ page }) => {
    const article = getNodeArticle(page, UNTIL);
    await expect(article).toBeVisible();
    await expect(article.locator('.graph-node__name')).toHaveText('GraphUntilLoopNode');
  });

  test('has the Loop category accent colour (#eab308)', async ({ page }) => {
    const color = await getNodeAccentColor(page, UNTIL);
    expect(color.toLowerCase()).toBe('#eab308');
  });

  test('has a state input port', async ({ page }) => {
    const inputs = await getNodePorts(page, UNTIL, 'in');
  ...
  test('required output port stateOut is visible (D2/G3-05)', async ({ page }) => {
    const outputs = await getNodePorts(page, UNTIL, 'out');
  ...
  test('double-click opens the node edit modal', async ({ page }) => {
    await openNodeEditor(page, UNTIL);
