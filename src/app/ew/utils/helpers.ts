...
export function revertGS1ToDateString(date: string, locale: string = 'pt-PT', fullDate: boolean = true): string {
  if (!date) return '';
...
export function getDoucumentOptions(type: 'product' | 'batch' = 'product'): SelectOption[] {
  return Object.values(DocumentsTypes)
    .map((doc) => ({
      text: doc.toLowerCase(),
      value: doc,
      disabled: doc === DocumentsTypes.SMPC,
      selected: doc === DocumentsTypes.leaflet,
    }))
    .filter((doc) => {
      if (type === 'batch') return doc.value !== DocumentsTypes.prescribingInfo;
      return doc;
    });
}

export function getMarketOptions(): SelectOption[] {
