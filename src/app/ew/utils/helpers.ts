...
export function getLeafletLanguages(): SelectOption[] {
  return languages.sort(a => a.code === 'en' ? - 1 : 1).map(language =>
    ({text: language.name, value: language.code.toLowerCase(), disabled: false, selected: language.code === "en"})
  );
}
...
export function getMarkets(): SelectOption[] {
  return countries.map(country =>
    ({text: country.name, value: country.code.toLowerCase(), disabled: false, selected: country.code === "us"})
  );
}
