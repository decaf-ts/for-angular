...
export async function createAuditHandler<M extends Model, R extends Repository<M, any>, V>(
  ...
  const toCreate = new Audit({
    ...
    transaction: id,
    table: Model.tableName(this.class),
...
export async function createUuidHandler<M extends Model, R extends Repository<M, any>, V>(
  this: R,
  context: any,
  data: V,
  key: keyof M,
...
export function uuid(update: boolean = false) {
  const decs: any[] = [generated('uuid'), type([String]), onCreate(createUuidHandler as any, {})];
