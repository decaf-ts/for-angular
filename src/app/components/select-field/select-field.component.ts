...
import { IonButton, IonItem, IonLabel, IonSelect, IonSelectOption, IonText, SelectChangeEventDetail, SelectCustomEvent } from '@ionic/angular/standalone';
...
export class AppSelectFieldComponent extends CrudFieldComponent {
  ...
  async setProductValue(batchId: string) {
    ...
    if(repo && this.value) {
      ...
      .eq(this.value).or(Condition.attribute<Model>('batchNumber' as keyof Model).eq(batchId));
